...
// Dynamic router and proxy mapping logic
router.all("/site/:slug/*", async (req, res) => {
  ...
  if (activeProcess) {
    if (activeProcess.status === "running") {
      ...
      // Rewrite request rewrite paths to feed clean context parameters down to child process
      req.url = req.url.replace(`/site/${slug}`, "") || "/";
